...
// session list rehydrates from sqlite via getTodaysSets) and, as a second,
...
test('a set logged with the network cut survives a reload', async ({ page, context }) => {
  ...
  // RIR left blank on purpose — FR-LOG-1 must not block submission on it.
  await page.getByTestId('log-set-button').click();

  const loggedRow = page.getByTestId('logged-set-row');
  await expect(loggedRow).toHaveCount(1);
  await expect(loggedRow).toContainText('62.5');
  await expect(loggedRow).toContainText('8');
  ...
  await page.getByRole('link', { name: /log a set/i }).click();
  await expect(page.getByTestId('logged-set-row')).toHaveCount(1);
  await expect(page.getByTestId('logged-set-row')).toContainText('62.5');
  await expect(page.getByTestId('logged-set-row')).toContainText('8');
