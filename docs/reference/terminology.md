...
- **Claim cluster** — two or more opposing `contested` claim records sharing a `clusterId`. Search and the Hub present the sides together; the current single-card workout picker does not preserve that grouped presentation.
...
- **JSON Logic subset** — the allowlisted predicate grammar used to match claims against a snapshot. Unknown operators, variables, unsafe nullable comparisons, and unsupported nesting are rejected; runtime errors fail closed.
...
- **Review ledger** — the non-runtime curation record for DOI checks, source locations, null rationales, grade reasoning, hostile reviews, and next review. Its presence does not imply every entry is complete.
...
- **Tie-to-silence** — the general surface selector's rule that equal top-ranked claims produce no card instead of letting array order make an evidence choice.
